...
public class Car {
    ...
    let _make: CarMake
    var _color: String // შეიძლება გადავღებოთ :}
    let _doorsCount: Int
    ...
    init(make carMake: CarMake, color carColor: String, doorsCount carDoors: Int, fuelInfo carFuel: FuelExpenses) {
        self._make = carMake
        self._color = carColor
        self._doorsCount = carDoors
        self._fuel = carFuel
    }
    
    public class func randomCar(withColor color: String) -> Car {
        let randomCarGenerated = Car.randomCar()
        randomCarGenerated.color = color
        return randomCarGenerated
    }
    ...
    public func runInKmWORefuel() -> Double {
        return (self._fuel.maxVolume / self._fuel.fuelNeededPerHundred * 100).rounded(toPlaces: 2)
    }
}
